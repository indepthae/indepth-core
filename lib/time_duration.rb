class TimeDuration
  def self.formatted_string(start_date, end_date, format_string='%y, %M, %w, %d and %h:%m:%s')
    self.diff(start_date, end_date, format_string)[:diff]
  end
  
  def self.diff(start_date, end_date, format_string)
  ...
  def TimeDuration.format_date_time(time_diff_components, format_string)
  ...
  def TimeDuration.pluralize(word, count)
    return count != 1 ? I18n.t(word.pluralize, :default => word.pluralize) : I18n.t(word, :default =>  word)
  end

  def TimeDuration.remove_format_string_for_zero_components(time_diff_components, format_string)
  ...
  def TimeDuration.format_digit(number)
    return '%02d' % number
  end
end