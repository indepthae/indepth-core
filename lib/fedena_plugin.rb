...
class FedenaPlugin
  ...
  def self.register=(plugin_details)
    unless AVAILABLE_MODULES.collect{|mod| mod[:name]}.include?(plugin_details[:name])
      ...
      ADDITIONAL_LINKS[:online_exam_index_link].flatten!
      ADDITIONAL_LINKS[:configuration_index_link] << plugin_details[:configuration_index_link] unless plugin_details[:configuration_index_link].blank?
      ADDITIONAL_LINKS[:icon_class_link] << plugin_details[:icon_class_link] unless plugin_details[:icon_class_link].blank?
      ...
      ADDITIONAL_LINKS[:login_hook]<<plugin_details[:login_hook] unless plugin_details[:login_hook].blank?
      ADDITIONAL_LINKS[:generic_hook]<<plugin_details[:generic_hook] unless plugin_details[:generic_hook].blank?
      ADDITIONAL_LINKS[:generic_hook].flatten!
      ADDITIONAL_LINKS[:generic_multi_hook]<<plugin_details[:generic_multi_hook] unless plugin_details[:generic_multi_hook].blank?
      ADDITIONAL_LINKS[:generic_multi_hook].flatten!
      FINANCE_CATEGORY << plugin_details[:finance] unless plugin_details[:finance].blank?
      FINANCE_CATEGORY.flatten!
      ...
      Authorization::AUTH_DSL_FILES << "#{RAILS_ROOT}/vendor/plugins/#{plugin_details[:name]=='fedena_google_sso' ? 'fedena_oauth' : plugin_details[:name]}/#{plugin_details[:auth_file]}" unless plugin_details[:auth_file].blank?
      if defined? plugin_details[:name].camelize.constantize
        ...
        if plugin_details[:name].camelize.constantize.respond_to? "student_profile_fees_hook"
          REGISTERED_HOOKS[:student_profile_fees] << plugin_details[:name]
        end

        if plugin_details[:name].camelize.constantize.respond_to? "mobile_student_profile_fees_hook"
          REGISTERED_HOOKS[:mobile_student_profile_fees] << plugin_details[:name]
        end
        ...
        if plugin_details[:name].camelize.constantize.respond_to? "event_alert_form"
          REGISTERED_HOOKS[:event_alert_form] << plugin_details[:name]
        end
        if plugin_details[:name].camelize.constantize.respond_to? "transport_admission_hook"
          REGISTERED_HOOKS[:student_admission] << plugin_details[:name]
        end
      end
    end
  end
  ...
  def self.check_dependency(record,action)
    ...
    AVAILABLE_MODULES.each do |mod|
      ...
      if modu.respond_to?("dependency_check")
        dependency << mod[:name] if modu.send("dependency_check",record,action).present?
      end
    end
    dependency
  end
  ...
  def self.plugin_installed?(plugin_name)
    AVAILABLE_MODULES.map{|m| m[:name]}.include?(plugin_name)
  end
