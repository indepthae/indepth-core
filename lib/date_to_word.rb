...
module DateToWord
  ...
  def self.day_to_words(d)
    ...
    if day.to_i > 20
      first = digits[day.first.to_i*10]
      second = single_days[day.last.to_i]
      "#{first} #{second}"
    else
      "#{single_days[day.to_i]}"
    end
  end
