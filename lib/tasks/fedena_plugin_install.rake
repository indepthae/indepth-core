    ...
    namespace :db do
      ...
      task :migrate => :environment do
        FedenaPlugin::AVAILABLE_MODULES.each do |m|
          ...
          ActiveRecord::Migrator.migrate("vendor/plugins/#{m[:name]=='fedena_google_sso' ? 'fedena_oauth' : m[:name]}/db/migrate/", ENV["VERSION"] ? ENV["VERSION"].to_i : nil)
              ...
              arr.each do |arr_l|
                begin
                  ...
                  if rec.present? and rec[file_type].present?
                    ...
                    if File.exists? file
                      rec.instance_eval do
                        attachment_for(d.first.to_sym).assign(open(file,'r'))
                        save_attached_files
                        if rec.errors.blank? && update_without_callbacks==1
