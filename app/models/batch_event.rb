...
class BatchEvent < ActiveRecord::Base
  belongs_to :batch
  belongs_to :event
  attr_accessor :batch_name, :selected
  ...
  def parent_event_emails
    ...
    students.each do |s| email=email+(s.immediate_contact.present?? (s.immediate_contact.email.to_s.zip(s.immediate_contact.first_name)):[]) ;end
    return email
  end
end
