...
class MonthlyPayslip < ActiveRecord::Base
  ...
  validates_numericality_of :amount,:allow_nil=>true,:allow_blank=>true
  ...
  def amount
    self[:amount] = self[:amount].present? ? self[:amount] : 0.0
  end

  def validate
    errors.add(:employee_id, :not_found) unless self.employee.present?
  end
  ...
  def self.total_employees_salary(start_date,end_date,dept_id="")
    ...
    unless total_monthly_payslips.blank?
      total_monthly_payslips.each do |payslip|
        total_salary += payslip.amount.to_f if payslip.payroll_category and payslip.payroll_category.is_deduction==false
        total_salary -= payslip.amount.to_f if payslip.payroll_category and payslip.payroll_category.is_deduction==true
      end
    end
  ...
  def self.fetch_employee_payslip_data(params)
    employee_payslip_data params
  end
end
