...
class ExamGroup < ActiveRecord::Base
  ...
  named_scope :active, :joins=>:batch, :conditions=>{:batches=>{:is_active=>true, :is_deleted=>false}}
  named_scope :inactive, :joins=>:batch, :conditions=>{:batches=>{:is_active=>false, :is_deleted=>false}}

  validates_presence_of :name
  belongs_to :batch
  has_one :grouped_exam
  belongs_to :course_exam_group
  has_many :exams, :dependent => :destroy
  before_destroy :removable?
  belongs_to :cce_exam_category
  belongs_to :icse_exam_category
  has_many :exam_group_fa_statuses
...
#  after_save :verify_update_and_send_sms
  ...
  def fa1_status
    return true  if exam_group_fa_statuses.first(:conditions=>{:fa_group=>"FA1"}).present?
    return false
  end
  def fa2_status
    return true  if exam_group_fa_statuses.first(:conditions=>{:fa_group=>"FA2"}).present?
    return false
  end
  def fa3_status
    return true  if exam_group_fa_statuses.first(:conditions=>{:fa_group=>"FA3"}).present?
    return false
  end
  def fa4_status
    return true  if exam_group_fa_statuses.first(:conditions=>{:fa_group=>"FA4"}).present?
    return false
  end
  ...
  def is_subject_teacher_for_this_subject
    ...
    if employee.subjects.present?
      user_ids = []
      exams.each do |exam|
        user_ids += exam.subject.employees.collect{|e| e.user.id}
      end      
  ...
  def verify_update_and_send_sms
    AutomatedMessageInitiator.exam_schedule_published(self)
  end
  ...
  def self.get_hash_priority
    hash = {:exam_detail=>[:subject,:start_time,:end_time,:maximum_marks,:minimum_marks,:event]}
    return hash
  end
  ...
  def build_exams_from_attributes (attrs)
    exams(true)
    attrs.each do |_,attr|
      _destroy = attr.delete("_delete")
      exam = exams.build(attr)
      exam['_delete']=_destroy
    end
  end
