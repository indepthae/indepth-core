...
class ExamScore < ActiveRecord::Base
  ...
  validate :grading_level_belongs_to_batch  ,:if=>Proc.new{|score| score.grading_level_id.present?}
  before_validation :fix_grading_level,:if=>Proc.new{|score| score.grading_level_id.present?}
  ...
  # Try to fix grading level if invalid grading level with same name is applied
  def fix_grading_level
    if GradingLevel.with_name(grading_level.name).size > 1
      self.grading_level = GradingLevel.with_name(grading_level.name).for_batch(student.batch_id).first if GradingLevel.with_name(grading_level.name).for_batch(student.batch_id).first
    end
  end
  def grading_level_belongs_to_batch
    unless grading_level.applicable_for_batch?(student.batch_id)
      errors.add_to_base(:given_grading_level_is_not_applicable_for_this_batch)
      return false
    end
  end

  def calculate_percentage
    exam.maximum_marks.to_f == 0 ? 0.0 : self.marks.to_f * 100 / self.exam.maximum_marks.to_f
  end

  def grouped_exam_subject_total(subject,student,type,batch = "",from_csv=nil)
    if batch == ""
    ...
    unless from_csv.present?
      if Authorization.current_user.parent? or Authorization.current_user.student?
  ...
  def calculate_grade
    ...
    unless exam_type == 'Grades'
      unless self.marks.nil? or exam.maximum_marks.to_f == 0
