...
class ClassTiming < ActiveRecord::Base
  ...
  named_scope :all_active_timings, :conditions => {:is_deleted => false}, :order => 'start_time ASC'
  before_destroy :check_dependency

  def check_dependency
    if self.class_timing_set.time_table_class_timing_sets.present?
  ...
  def validate
    ...
    start_overlap = class_timing_set.class_timings.find(:all,:conditions=>["start_time < time(?) and end_time > time(?) and is_deleted = ?", start_time,start_time,false]).reject{|ct| ct.id == id}.present?
    end_overlap = class_timing_set.class_timings.find(:all,:conditions=>["start_time < time(?) and end_time > time(?) and is_deleted = ?", end_time,end_time,false]).reject{|ct| ct.id == id}.present?
    between_overlap = class_timing_set.class_timings.find(:all,:conditions=>["start_time < time(?) and end_time > time(?) and is_deleted = ? ",end_time, start_time,false]).reject{|ct| ct.id == id}.present?
    ...
    errors.add_to_base(:timetable_entries_present) if(self.timetable_entries.present? and self.is_break==true)
  end
end
