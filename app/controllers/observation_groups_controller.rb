...
class ObservationGroupsController < ApplicationController
  ...
  def index
    @obs_groups=ObservationGroup.active.all(:order=>'sort_order ASC')
  end
  ...
  def new
    ...
    @di_count = CceReportSetting.get_setting_value('DICount').to_i
  end

  def create
    @obs_group=ObservationGroup.new(params[:observation_group])
    @obs_group.sort_order=ObservationGroup.find(:last,:conditions=>{:is_deleted=>false},:order=>"sort_order ASC").try(:sort_order).to_i+1 || 1
    if @obs_group.save
  ...
  def edit
    ...
    @grade_sets=CceGradeSet.all
    @observation_kinds=ObservationGroup::OBSERVATION_KINDS
    @di_count = @obs_group.di_count_in_report || CceReportSetting.get_setting_value('DICount').to_i
  end
  ...
  def destroy
    ...
    unless @category.observations.active.present?
  ...
  def update_course_obs_groups
    ...
    @course_observation_groups=@course.observation_groups.active.all(:order=>'sort_order ASC')
    @obs_groups=ObservationGroup.active.all(:order=>'sort_order ASC')
    flash[:notice] = "Observation groups settings saved"
  ...
  def destroy_observation
    @observation=Observation.find(params[:id])
    @obs_group=@observation.observation_group
    unless @observation.descriptive_indicators.present?
  ...
  def reorder_ob_groups
    if request.post?
      swap=ObservationGroup.active.all(:order=>"sort_order ASC")
      initial=params[:count].to_i
      src=swap[initial]
      if params[:direction]=='up'
        dest=swap[initial-1]
