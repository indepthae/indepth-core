...
class CceWeightagesController < ApplicationController
  ...
  check_request_fingerprint :create,:update_course_weightages
  def index
  ...
  def destroy
    ...
    if @weightage.courses.empty?
      ...
      flash[:warn_notice]="CCE weightage #{@weightage.weightage}(#{@weightage.criteria_type}) has been assigned to classes. Remove the associations before deleting."
    end
  ...
  def update_course_weightages
    ...
    @course.cce_weightages = CceWeightage.find_all_by_id(params[:course][:weightage_ids]) if params[:course].present? and params[:course][:weightage_ids].present?
    @course_weightages=@course.cce_weightages
    @weightages=CceWeightage.all
    if params[:course].present? and params[:course][:weightage_ids].present? and @course.save
      flash[:notice] = "CCE weightages for the class assigned successfully."
