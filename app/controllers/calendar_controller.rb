...
class CalendarController < ApplicationController
  ...
  def index
    ...
    first_day = @show_month.beginning_of_month.beginning_of_day
    last_day =  @show_month.end_of_month.end_of_day
    if @user.admin? or privilege.include?("EventManagement")
      @events = Event.all(:conditions =>["((ffc.id IS NULL OR (fa_fc.id IS NULL OR fa_fc.is_deleted = false)) AND
                          ...
                          first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:origin],
    ...
    if @user.employee? and !privilege.include?("EventManagement")
      @events = Event.all(:conditions =>["((ffc.id IS NULL OR (fa_fc.id IS NULL OR fa_fc.is_deleted = false)) AND
                          ...
                          (events.start_date BETWEEN ? AND ?) or (events.end_date BETWEEN ? AND ?) or
                          (? BETWEEN events.start_date AND events.end_date) or (? BETWEEN events.start_date AND events.end_date)",
                          first_day,last_day,first_day,last_day,first_day,last_day], :include=>[:origin,:employee_department_events],
                          :joins => "LEFT OUTER JOIN finance_fee_collections ffc ON ffc.id = origin_id AND origin_type = 'FinanceFeeCollection'
                   LEFT OUTER JOIN fee_accounts fa_fc ON fa_fc.id = ffc.fee_account_id
                   LEFT OUTER JOIN hostel_fee_collections hfc ON hfc.id = origin_id AND origin_type = 'HostelFeeCollection'
                   LEFT OUTER JOIN fee_accounts fa_hfc ON fa_hfc.id = hfc.fee_account_id
    ...
    if @user.student? or @user.parent?
      @events = Event.all(:conditions =>["((ffc.id IS NULL OR (fa_fc.id IS NULL OR fa_fc.is_deleted = false)) AND
                          ...
                          (events.start_date BETWEEN ? AND ?) or (events.end_date BETWEEN ? AND ?) or
                          (? BETWEEN events.start_date AND events.end_date) or (? BETWEEN events.start_date AND events.end_date)",
                          first_day,last_day,first_day,last_day,first_day,last_day], :include=>[:origin,:batch_events],
                          :joins => "LEFT OUTER JOIN finance_fee_collections ffc ON ffc.id = origin_id AND origin_type = 'FinanceFeeCollection'
                   LEFT OUTER JOIN fee_accounts fa_fc ON fa_fc.id = ffc.fee_account_id
                   LEFT OUTER JOIN hostel_fee_collections hfc ON hfc.id = origin_id AND origin_type = 'HostelFeeCollection'
                   LEFT OUTER JOIN fee_accounts fa_hfc ON fa_hfc.id = hfc.fee_account_id
  ...
  def new_calendar
    ...
    first_day = @show_month.beginning_of_month.beginning_of_day
    last_day =  @show_month.end_of_month.end_of_day
    @events = Event.all(:conditions =>["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include => :origin)
    load_notifications
  ...
  def show_event_tooltip
    ...
    first_day = @date.beginning_of_month.beginning_of_day
    last_day = @date.end_of_month.end_of_day
    ...
    non_common_events = Event.find_all_by_is_common_and_is_holiday_and_is_exam_and_is_due(false,false,false,false, :conditions => ["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day])
    @common_event_array = []
  ...
  def show_holiday_event_tooltip
    ...
    non_common_holiday_events = Event.find_all_by_is_common_and_is_holiday(false,true, :conditions => ["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:batch_events,:employee_department_events])
  ...
  def show_exam_event_tooltip
    ...
    subject_ids = []
    if @user.student == true or @user.parent == true
      not_common_exam_event = Event.find_all_by_is_common_and_is_holiday_and_is_exam(false,false,true, ["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:origin,:batch_events])
      user_student = @user.student_record if @user.student
      user_student = @user.parent_record if @user.parent
      ...
      not_common_exam_event.reject! { |x|x.origin.nil? or ! subject_ids.include? x.origin.subject_id or !x.is_published_exam }
      batch = user_student.batch
      ...
      not_common_exam_event = Event.find_all_by_is_common_and_is_holiday_and_is_exam(false,false,true, :conditions => ["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:origin])
      not_common_exam_event.reject! { |x|x.origin.nil? or !x.is_published_exam }
      not_common_exam_event.each do |h|
        if  h.start_date.to_date == h.end_date.to_date
  ...
  def show_due_tooltip
    ...
    elsif @user.employee? and !privilege.include?("EventManagement") and !privilege.include?("Librarian")
      finance_due_check.reject!{|x| !x.is_employee_event(@user) }
    end
    @finance_due = []
    finance_due_check=Hash[*(finance_due_check).map {|obj| [obj.origin_id, obj]}.flatten].values
    ...
    @events.each do |e|
      ...
      if e.is_due == true
        if e.is_active_event
          ...
          elsif (privilege.include?("Librarian") and e.origin_type=='BookMovement')
            build_common_events_hash(e,'library_due',@show_month)
      ...
      if e.is_common ==false and e.is_holiday==false and e.is_exam==false and e.is_due==false   #not_common_event
        ...
        build_employee_events_hash(e,'employee_dept_not_common',@user.employee_record.employee_department_id,@show_month) if @user.employee? and !privilege.include?("EventManagement")
        unless e.origin.nil?
      ...
      if e.is_common ==false and e.is_holiday==true     # not_common_holiday_event
        ...
        build_employee_events_hash(e,'employee_dept_not_common_holiday',@user.employee_record.employee_department_id,@show_month) if @user.employee? and !privilege.include?("EventManagement")
        if @user.admin? or privilege.include?("EventManagement") 
