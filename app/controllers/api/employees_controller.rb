class Api::EmployeesController < ApiController
  ...
  def create
    ...
    begin
      employee_params_main = File.read(params[:employee].try(:path)).gsub(/\s&\s/,' &amp; ')
    rescue Exception => e
      render "single_access_tokens/500.xml", :status => :bad_request  and return
      puts e.message
    end
    ...
    employee_params.keys.each do |key|
      if Employee.reflect_on_all_associations.map(&:name).include? key.to_sym
        ...
        if key == "reporting_manager"
          reporting_manager = Employee.first(:conditions => ["employee_number LIKE BINARY(?)",employee_params[key]]).try(:user)
          employee_params[key] = reporting_manager
        end
      end
    end
    @employee = Employee.new(employee_params)
    @employee.photo = params[:employee_photo] if params[:employee_photo].present? and params[:employee_photo].class.to_s=="Tempfile"
    respond_to do |format|
      if employee_params_main.present?
        if employee_additional_details_params.present?
          ...
          additional_datas["additional_field"].to_a.each do |additional_data|
            additional_field = AdditionalField.active.find_by_name(additional_data["name"].squish)
        ...
        if employee_bank_details_params.present?
          ...
          bank_datas["bank_field"].to_a.each do |bank_data|
            bank_field = BankField.active.find_by_name(bank_data["name"].squish)
  ...
  def upload_photo
    ...
    if @employee.nil?
      ...
      @employee.photo = params[:photo] if params[:photo].present? and params[:photo].class.to_s="Tempfile"
      @employee.save
      respond_to do |format|
  ...
  def biometric
    @xml = Builder::XmlMarkup.new
    @employee = Employee.find_student_with_biometric(params[:id])
   
    respond_to do |format|
      if @employee.present?
        @employees =  @employee.first.get_profile_data
        format.xml  { render :employee }
      else
        render "single_access_tokens/500.xml", :status => :bad_request  and return
      end
