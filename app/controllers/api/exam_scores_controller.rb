class Api::ExamScoresController < ApiController
  ...
  def create
    ...
    student = Student.first(:conditions => ["admission_no LIKE BINARY(?)",params[:admission_no]]).try(:id)
    ...
    respond_to do |format|
      if (params[:exam_group_name].present? and params[:subject_code].present? and params[:admission_no].present? and params[:batch].present? and (params[:marks].present? or params[:grade].present?))
  ...
  def update
    ...
    student = Student.first(:conditions => ["admission_no LIKE BINARY(?)",params[:id]]).try(:id)
    grading_level = GradingLevel.for_batch(batch.try(:id)).find_by_name(params[:grade]).try(:id)
    grading_level ||= GradingLevel.default.find_by_name(params[:grade]).try(:id)
    @exam_score = exam.exam_scores.find_by_student_id(student) unless exam.nil?
    respond_to do |format|
      if (params[:exam_group_name].present? and params[:subject_code].present? and params[:id].present? and params[:batch].present? and (params[:marks].present? or params[:grade].present?))
        unless @exam_score.nil?
