...
class TimetableEntriesController < ApplicationController
  ...
  check_request_fingerprint :update_multiple_timetable_entries2,:tt_entry_update2
  ...
  def update_batch_list
    @tt=Timetable.find(params[:timetable_id], :include => :time_table_class_timings)
    batch_ids = @tt.time_table_class_timings.map {|x| x.batch_id }

    batches = Batch.find_all_by_id(batch_ids, :conditions => {:is_deleted => false, :is_active => true, :course_id => params[:course_id] },
      :joins=>:course,:select=>"`batches`.*,CONCAT(courses.code,'-',batches.name) as course_full_name",
      :order=>"course_full_name",:include=>[:course,:subjects,{:elective_groups => :subjects }])
    @batches = batches.select {|batch| batch.elective_groups.select{|elective_group| !elective_group.is_deleted and elective_group.subjects.select{|subject| !subject.is_deleted }.present?}.present? or batch.subjects.select{|subject| (!subject.is_deleted and !subject.elective_group_id.present?)}.present?}
    render :update do |page|
      page.replace_html "batches", :partial => "batch_list"
      page.replace_html "tutor_list", :text => ""
...
#      render :controller => :user, :action => :dashboard 
  ...
  def tt_entry_update2
    ...
    unless tte.nil?
      ...
      tte = TimetableEntry.create(:weekday_id=>weekday,:class_timing_id=>class_timing, :entry_id => entry_id,
        :entry_type => entry_type, :batch_id=>@batch.id,:timetable_id=>@timetable.id)
    end
    ...
    tte_from_batch_and_tt(@timetable.id)
    render :update do |page|
      page.replace_html 'tt_over',:partial=>'new_entry'
  ...
  def tte_from_batch_and_tt(tt)
    ...
    time_table_class_timings = TimeTableClassTiming.find_by_timetable_id_and_batch_id(@tt.id,@batch.id,:include => :time_table_class_timing_sets)
    ...
    if @tt.duration >= 7
      time_table_class_timing_sets = time_table_class_timings.try(:time_table_class_timing_sets)
      @weekday = weekday_arrangers(time_table_class_timing_sets.collect(&:weekday_id)) if time_table_class_timing_sets
    else
      weekdays=[]
    ...
    @all_timetable_entries=TimetableEntry.find(:all,:conditions=>{:batch_id=>@batch.id,:timetable_id=>@tt.id, :weekday_id => @tt.timetable_weekdays},
      :include=>[:entry,:employees,:timetable_swaps,:allocated_classrooms])
    ...
    @all_timetable_entries.each do |tte|
      ...
      @tte_total = @tte_total.next
    end
    @subjects = @batch.subjects.select { |x| (!x.elective_group_id.present? and !x.is_deleted) }
    @subjects = @subjects.sort_by{|sb| sb.priority.to_i}
    @elective_groups = @batch.elective_groups.select { |eg| (eg.subjects.select { |subject| !subject.is_deleted }.present? and !eg.is_deleted) }
    #    @elective_groups = ElectiveGroup.find_all_by
    @ele_subjects = @batch.subjects.select { |subject| (subject.elective_group_id.present? and !subject.is_deleted) }.group_by{|subject| subject.elective_group } #.group_by{|x| x.elective_group_id }
    @ele_subjects.each_pair do |k , v|
      v.sort_by{|v_el| v_el.priority.to_i}
    end
