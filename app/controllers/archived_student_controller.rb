...
class ArchivedStudentController < ApplicationController
  ...
  def profile
    ...
    @tc_presence =  TcTemplateRecord.find_by_student_id(params[:id])
  end
  ...
  def show
    @student = ArchivedStudent.first(:conditions => ["admission_no LIKE BINARY(?)",params[:id]])
  ...
  def guardians
    ...
    #    if @parents.empty?
    @parents =@archived_student.archived_guardians
    #    end
  end
  ...
  def fees
    ...
    @dates=[]
    if @student.former_has_paid_fees
      @flash_notice=t('do_not_create_fee_collections_from_now_on')
    elsif @student.former_has_paid_fees_for_batch
  ...
  def student_report
    ...
    unless @config.config_value == 'Daily'
      ...
      @student_leaves = Attendance.find(:all,  :conditions =>{:student_id=>@student.former_id,:batch_id=>@batch.id,:month_date => @start_date..@end_date})
      @academic_days=@batch.academic_days.select{|v| v<=@end_date}.count
      leaves_forenoon=Attendance.count(:all,:conditions=>{:student_id=>@student.former_id,:batch_id=>@batch.id,:forenoon=>true,:afternoon=>false,:month_date => @start_date..@end_date})
      leaves_afternoon=Attendance.count(:all,:conditions=>{:student_id=>@student.former_id,:batch_id=>@batch.id,:forenoon=>false,:afternoon=>true,:month_date => @start_date..@end_date})
      leaves_full=Attendance.count(:all,:conditions=>{:student_id=>@student.former_id,:batch_id=>@batch.id,:forenoon=>true,:afternoon=>true,:month_date => @start_date..@end_date})
  ...
  def generated_report
    if params[:student].nil?
      ...
      @graph = open_flash_chart_object(770, 350,
        "/archived_student/graph_for_generated_report?batch=#{@student.batch.id}&examgroup=#{@exam_group.id}&student=#{@student.former_id}")
    end
  end
  ...
  def generated_report3
    ...
    @exam_groups.reject!{|e| e.result_published==false}
    @graph = open_flash_chart_object(770, 350,
      "/archived_student/graph_for_generated_report3?subject=#{@subject.id}&student=#{@student.former_id}")
  end
