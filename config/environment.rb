...
Rails::Initializer.run do |config|
  ...
  config.load_paths += Dir["#{RAILS_ROOT}/app/presenters"]
  config.load_paths += ["#{RAILS_ROOT}/lib/message"]
  config.load_paths += Dir["#{RAILS_ROOT}/lib/gradebook"]
  ...
  if File.exists?('config/s3_assets_settings.yml')
    ...
    config.action_controller.asset_host = settings['access_url']
  end
