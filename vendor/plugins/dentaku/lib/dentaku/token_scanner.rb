...
module Dentaku
  class TokenScanner
    ...
    class << self
      ...
      def combinator
        new(:combinator, '(and|or)\b', lambda { |raw| raw.strip.to_sym })
      end

      def function
        new(:function, '\w+\s*\(', lambda do |raw|
          function_name = raw.gsub('(', '')
          [Token.new(:function, function_name.strip.to_sym, function_name), Token.new(:grouping, :fopen, '(')]
        end)
      end

      def identifier
        new(:identifier, '\w+\b', lambda { |raw| raw.strip.to_s })
      end
    end
  end
end
