...
module GoogleDrive
    ...
    class File
        ...
        #     @document_feed_url = "%s/%s" % ['https://www.googleapis.com/drive/v2/files', CGI.escape(self.resource_id)]
        #   end
        #   @acl = nil
        # end
        def initialize(session, entry_or_url) #:nodoc:
        ...
        def file_id
          return self.document_feed_entry["id"]
        end
        ...
        # Resource ID.
        def resource_id
          return self.document_feed_entry.css("kind").text
        end
        ...
        # Set <tt>params[:reload]</tt> to true to force reloading the title.
        def title(params = {})
          return document_feed_entry(params)["name"]
        end
        ...
        # end # URL to view/edit the file in a Web browser.
        #
        # e.g. "https://docs.google.com/file/d/xxxx/edit"
        def human_url
          return self.document_feed_entry["webViewLink"]
        end
        def download_file
          export_links=self.document_feed_entry["exportLinks"]
          unless self.document_feed_entry["webContentLink"].nil?
            unless self.document_feed_entry["webContentLink"].empty?
              url=self.document_feed_entry["webContentLink"]
            end
          end
          unless export_links.nil?
            unless export_links.empty?
              url=export_links["application/vnd.openxmlformats-officedocument.wordprocessingml.document"]|| export_links["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]|| export_links["application/vnd.openxmlformats-officedocument.presentationml.presentation"] || export_links.values.first
            end
          end
          url
        end
        ...
        def delete(permanent = false)
          ...
          #url = concat_url(url, "/trash") unless permanent
          method = permanent ? :delete : :patch
          data = permanent ? {} : {:trashed => true} 
          @session.request(method, url,
            :auth => :writely, :header => {"If-Match" => "*","Content-Type" => "application/json"},:data=>data.to_json)
        end
