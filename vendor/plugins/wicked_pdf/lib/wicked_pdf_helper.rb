module WickedPdfHelper
  ...
  def wicked_pdf_image_tag(img, options={})
    if(FedenaSetting.s3_enabled? and options[:s3].present? and options[:s3])      
      ...
      img_tag = (verify_http_https_file image_url) ? (image_tag image_url).gsub('&amp;','&') : ''
      return (img_tag.present? ? img_tag : default_image(img.path, options))
    else
      return default_image(img.path, options)
    end
  end

  def default_image image_path, options
    return File.exists?("#{Rails.root.join(image_path)}") ? 
      (image_tag "file://#{Rails.root.join(URI.encode(image_path))}", options) : ""
  end
