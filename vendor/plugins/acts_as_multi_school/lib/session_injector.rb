...
class SessionInjector
  ...
  DEFAULT_OPTIONS = {
    ...
    :key => ActionController::Session::AbstractStore::DEFAULT_OPTIONS["key"],
  ...
  def initialize(app, options = {})
    ...
    # get session_options for kay and secret
    # secret to be used in place of token key for the encryption of hs data
    session_options = get_session_options(@app)
    @session_id_key = options[:key] || session_options[:key]
    ...
    @token_key = options[:token_key] || session_options[:secret] || generated_token_key
  ...
  def get_session_options (middleware)
    middleware = get_session_store_app(middleware)
    middleware.instance_variable_get(:@default_options)
  end

  def get_session_store_app (middleware)
    middleware = middleware.instance_variable_get(:@app)
    return middleware.is_a?(ActiveRecord::SessionStore) ? middleware : get_session_store_app(middleware)
  end
