module Paperclip::FormHelper
  ...
  def default_file_upload_multiple(object_name, method, options = {})
    on_change=options[:onchange].to_s.dup    
    options.delete(:onchange)
    unique_id = options[:uni]
    options['onchange'] = "#{unique_id}.paperclip_file_upload_test1(event,this,'#{unique_id}');#{on_change};"
    label_field_name = "field_#{object_name.to_s.split('[').join('_').split(']').join('')}_#{method.to_s}"
    " <div class='paperclip_field'>
          <input id='#{label_field_name}' class='field1' type='text' style ='display :none' disabled value='"+t('no_file_selected')+"' default='"+t('no_file_selected')+"'>
              <div class='style'>"+
      ::ActionView::Helpers::InstanceTag.new(object_name, method, self, options.delete(:object)).to_input_field_tag("file", options).html_safe + "" +
      "</div><div class='remove_attachment' style ='display :none'><span class='remove_attachment_tag' onclick="+"#{unique_id}.remove_element(this,'#{unique_id}');></span></div></div><script> window.#{unique_id} = new PaperclipHelp; </script>"
  end 
  ...
  def paperclip_file_field_tag_multiple(object_name, method, options = {})
      object = options[:object]
      unique_id = options[:uni]
      if is_s3_enabled?(object,method)
        max_file_size = object.send(method).instance_variable_get '@max_file_size'
        ...
        on_change=options[:onchange].to_s.dup
        options.delete(:onchange)
#        id = objectnew_message_name.gsub("[","_").gsub("]","_")
        ...
        paperclip_field = "
          <div class='paperclip_field'>
            <input id='#{label_field_name}' class='field1' type='text' style ='display :none' disabled disabled value='"+t('no_file_selected')+"' default='"+t('no_file_selected')+"' >
            <div class='pgbar' id='#{progressbar_name}'>
              <div class='outpg' id='#{progressbar_out_name}'>
                 <div class='pg' id='#{progressbar_in_name}'></div>
              </div>
        ...
        if !options[:direct] or options[:direct] == 'false'
          ...
          paperclip_field += "</div><div class='remove-field' style ='display :none' id = 'remove-field-'><a onclick= "+"#{unique_id}.remove_uploaded_file_test(this,'#{unique_id}');></a></div></div><script> window.#{unique_id} = new PaperclipHelp; </script>"

    else
        default_file_upload_multiple(object_name, method, options)
    end
