...
require 'aws_extension'   if (File.exist? File.join(::Rails.root, 'config', 'amazon_s3.yml'))
class RedactorS3Helper
  ...
  def rename(s3_object_key,s3_new_object_key)
    response_code = AWS::S3::S3Object.rename(s3_object_key,s3_new_object_key,Config.bucket_public,{:copy_acl => true})
#    AWS::S3::S3Object.delete(s3_object_key,Config.bucket_public)
    
    if(response_code)
      "https://#{Config.cloudfront_public}/#{URI.encode(s3_new_object_key).gsub('+','%2B')}"
    end
  end
