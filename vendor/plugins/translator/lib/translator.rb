...
module I18n
  class << self
    ...
    def translate_with_options(*args)
      options = args.extract_options!
      key = args.first
      self.default_locale = 'en-ch'
      unless (key.to_s.include? 'date.formats.') or (key.to_s.include? 'time.formats.')
        options.merge!(CustomTranslation.translate_options||{})
      end

      translate_without_options(*(args+[options]))
    end

    alias_method_chain :translate, :options
